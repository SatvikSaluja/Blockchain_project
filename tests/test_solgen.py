...
def test_generated_reproducer_compiles_and_passes():
    ...
    with AnvilProcess(port=8635) as anvil:
        ...
        source = generate_reproducer_sol(candidate, dep, seed=scenario.seed)

    out_path = GENERATED_DIR / f"ExploitReproducer_{scenario.seed}.t.sol"
    GENERATED_DIR.mkdir(parents=True, exist_ok=True)
    out_path.write_text(source)
    try:
        result = subprocess.run(
            ["forge", "test", "--match-contract", f"ExploitReproducer_{scenario.seed}"],
            cwd=REPO_ROOT,
            capture_output=True,
            text=True,
            env=os.environ,
